  ...
  public static function index($ssidName, $macToCheck) {
    ...
    if ($exists) {
      return [
        "status" => true,
        "ssid"   => $ssidName,
        "message" => "⚠ MAC $macToCheck already exists in SSID '$ssidName' filter list.",
        "policy"  => $macFilterPolicy,
    ...
    if ($updateResult && $updateResult->meta->rc === "ok" && $httpCode === 200) {
      return [
        "status" => true,
        "ssid"   => $ssidName,
        "message" => "✅ MAC $macToCheck successfully added to SSID '$ssidName' filter list.",
        "policy"  => $macFilterPolicy,
        "filter_enabled" => $macFilterEnabled
      ];
    } else {
