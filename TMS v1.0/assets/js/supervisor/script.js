...
const TEAM_MEMBERS = [
  {name:"Jamie Diaz", initials:"JD", color:"linear-gradient(155deg,#C98A2E,#B75B39)", role:"Team Member"},
  {name:"Marcus Lin", initials:"ML", color:"linear-gradient(155deg,#2F6F5E,#1F5647)", role:"Team Member"},
  {name:"Elena Cruz", initials:"EC", color:"linear-gradient(155deg,#8A6FB0,#6A4F90)", role:"Team Member"},
  {name:"Noah Bennett", initials:"NB", color:"linear-gradient(155deg,#A23B3B,#7E2E2E)", role:"Team Member"},
];
function memberInfo(name){
  if(name===CURRENT_USER) return {name:CURRENT_USER, initials:"PS", color:"linear-gradient(155deg,#3B6FA0,#274F72)"};
...
function renderWorkload(){
  ...
  strip.innerHTML = TEAM_MEMBERS.map(m=>{
...
function openModal(){
  ...
  assigneeSelect.innerHTML = TEAM_MEMBERS.map(m=>`<option value="${m.name}">${m.name}</option>`).join('');
