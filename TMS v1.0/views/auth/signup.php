...
<style>
  ...
  *{box-sizing:border-box;}
  ...
  .pw-bar-track{display:flex;gap:4px;margin-bottom:7px;}
  .pw-bar-seg{flex:1;height:4px;border-radius:3px;background:var(--surface-alt);}
  .pw-bar-seg.on-weak{background:var(--red);}
  .pw-bar-seg.on-fair{background:var(--amber);}
  .pw-bar-seg.on-strong{background:var(--accent);}
  .pw-req-list{display:flex;flex-wrap:wrap;gap:6px 14px;}
  .pw-req{display:flex;align-items:center;gap:5px;font-size:11px;color:var(--ink-faint);}
  .pw-req svg{width:12px;height:12px;flex-shrink:0;}
  .pw-req.met{color:var(--accent-dark);}
  .pw-req.met svg{color:var(--accent);}

  .checkbox-row{display:flex;align-items:flex-start;gap:8px;font-size:12px;color:var(--ink-soft);line-height:1.5;margin-bottom:20px;}
  .checkbox-row input{width:15px;height:15px;accent-color:var(--accent);margin-top:2px;flex-shrink:0;}
  .checkbox-row a{color:var(--accent-dark);font-weight:600;}
  ...
  @media (max-width:520px){
    ...
    .auth-card{
      ...
      display:flex;flex-direction:column;justify-content:center;padding:32px 22px;
    }
...
<div class="auth-card">
  ...
  <div class="form-success" id="formSuccess">
    ...
    <span>Account details look good — this demo would POST to <code>/api/auth/register.php</code> from here.</span>
  </div>
  ...
  <form id="signupForm">
    ...
    <div class="field-row2">
      <div class="field-group">
        ...
        <div class="input-wrap">
          <input type="password" id="suPassword" class="has-toggle" placeholder="Create a password" autocomplete="new-password">
      ...
      <div class="field-group">
        ...
        <div class="input-wrap">
          <input type="password" id="suConfirm" class="has-toggle" placeholder="Repeat password" autocomplete="new-password">
    ...
    <div class="pw-strength">
      <div class="pw-bar-track" id="pwBarTrack">
        <div class="pw-bar-seg"></div><div class="pw-bar-seg"></div><div class="pw-bar-seg"></div><div class="pw-bar-seg"></div>
      </div>
      <div class="pw-req-list" id="pwReqList">
        <span class="pw-req" data-req="len"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M9 11l3 3L22 4"/><path d="M21 12v7a2 2 0 01-2 2H5a2 2 0 01-2-2V5a2 2 0 012-2h11"/></svg>8+ characters</span>
        <span class="pw-req" data-req="upper"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M9 11l3 3L22 4"/><path d="M21 12v7a2 2 0 01-2 2H5a2 2 0 01-2-2V5a2 2 0 012-2h11"/></svg>1 uppercase</span>
        <span class="pw-req" data-req="num"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M9 11l3 3L22 4"/><path d="M21 12v7a2 2 0 01-2 2H5a2 2 0 01-2-2V5a2 2 0 012-2h11"/></svg>1 number</span>
      </div>
    </div>
    <br>
  ...
  <div class="divider-row"><span class="line"></span><span>Already have a workspace</span><span class="line"></span></div>
...
document.getElementById('suPassword').addEventListener('input', e=>{
  const {checks, score} = evaluatePassword(e.target.value);
  ...
  let level = 'weak';
  if(score >= 4) level = 'strong'; else if(score >= 3) level = 'fair';
  segs.forEach((seg, i)=>{
